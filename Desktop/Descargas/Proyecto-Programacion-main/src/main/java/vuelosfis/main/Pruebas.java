...
public class Pruebas {
    ...
    public static void main(String[] args) {
        ...
        // --- PRUEBA DE INTEGRACIÓN (CONTROLADOR + ARCHIVOS) ---
        System.out.println("--- INICIANDO CONTROLADOR ---");
        ControladorReserva control = new ControladorReserva();

        // 1. Cargar (Debe leer el archivo vuelos.txt)
        control.cargarDatosIniciales();

        // 2. Buscar (Probamos la lógica de filtrado)
        System.out.println("\nBuscando vuelos UIO -> MAD...");
        var resultados = control.buscarVuelos("UIO", "MAD");

        for (var v : resultados) {
         System.out.println("Encontrado: " + v.getCodigo() + " - $" + v.getPrecioBase());
            }

// 3. Guardar (Usamos una reserva falsa para ver si escribe el CSV)
// (Aquí puedes reutilizar una reserva creada en tus pruebas anteriores)
// control.finalizarReserva(reservaFalsa);
